...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  // Find original category name with proper casing
  const categoryName = filteredPosts.length > 0 && filteredPosts[0].categories
    ? filteredPosts[0].categories.find(
      c => c.toLowerCase() === normalizedCategory
    )
    : normalizedCategory.replace(/\b\w/g, l => l.toUpperCase()); // Capitalize if not found
